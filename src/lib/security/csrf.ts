...
export function setCsrfTokenCookie(response: NextResponse, token: string): NextResponse {
  try {
    ...
    logger.error({ error }, 'Error setting CSRF token cookie')
    return response
  }
}
...
export function clearCsrfTokenCookie(response: NextResponse): NextResponse {
  try {
    ...
    logger.error({ error }, 'Error clearing CSRF token cookie')
    return response
  }
}
...
export const csrfMiddleware: SecurityMiddleware = async (
  ...
  try {
    ...
    logger.error({ error, pathname: request.nextUrl.pathname }, 'Error in CSRF middleware')
    return response
  }
}
...
export function refreshCsrfToken(_request: NextRequest, response: NextResponse): NextResponse {
  try {
    ...
    logger.error({ error }, 'Error refreshing CSRF token')
    return response
  }
}
