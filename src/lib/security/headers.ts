...
export function applySecurityHeaders<T = unknown>(
  ...
  try {
    const { generateNonce = false, strictCSP = false, customHeaders = {} } = options

    // Generate nonce if requested
    const nonce = generateNonce ? generateCSPNonce() : undefined
...
export function setSecureCookie(
  ...
  try {
    ...
    logger.error({ error, cookieName: name }, 'Error setting secure cookie')
  }
}
...
export function clearSecureCookie(response: NextResponse, name: string): void {
  try {
    ...
    logger.debug({ cookieName: name }, 'Secure cookie cleared')
  } catch (error) {
    logger.error({ error, cookieName: name }, 'Error clearing secure cookie')
  }
}
...
export function withSecurityHeaders<T extends unknown[]>(
  ...
  return async (...args: T): Promise<NextResponse> => {
    try {
      ...
      logger.error({ error }, 'Error in security headers middleware')
...
export function applyCorsHeaders(response: NextResponse, origin?: string): NextResponse {
  try {
    ...
    logger.debug(
      {
        origin,
        allowedOrigin: corsConfig.origins.includes(origin || ''),
    ...
    logger.error({ error, origin }, 'Error applying CORS headers')
    return response
  }
}
 ...
 */
export const securityHeadersMiddleware: SecurityMiddleware = async (
  request: NextRequest,
  response: NextResponse
  ...
  try {
    ...
    logger.error({ error, pathname: request.nextUrl.pathname }, 'Error in security headers middleware')
    return response
  }
}
